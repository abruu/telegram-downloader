...
_Built for Jellyfin · Plex · Emby — with first-class support for Malayalam, Tamil, Telugu, and all regional Indian cinema_
...
- Asks for VPN configuration (optional)
- Writes `.env`
...
# Override VPN setting at runtime:
python3 tg_downloader_bot.py --vpn=true   # force VPN on
python3 tg_downloader_bot.py --vpn=false  # force VPN off
```
...
│   └── vpn.py              # OpenVPN connection manager
│
...
| `VPN_ENABLED`          | Optional    | Enable OpenVPN tunnel (default: `false`)                      | `true`                     |
| `VPN_CONFIG_FILE`      | If VPN      | Path to .ovpn configuration file                              | `/etc/openvpn/client.ovpn` |
| `VPN_AUTH_FILE`        | Optional    | Path to credentials file (username\npassword)                 | `/etc/openvpn/auth.txt`    |
| `VPN_RECONNECT`        | Optional    | Auto-reconnect on tunnel drop (default: `true`)               | `true`                     |
| `VPN_MAX_RETRIES`      | Optional    | Max reconnect attempts, 0=unlimited (default: `5`)            | `5`                        |
...
- Python dependencies (telethon, python-telegram-bot, etc.)
- Systemd service
- OpenVPN (optional)
...
<details>
<summary><b>VPN connection fails at startup</b></summary>

Check:

1. OpenVPN is installed: `which openvpn`
2. Config file exists: `ls -l /path/to/your.ovpn`
3. Bot has permissions to create tun interfaces (may need `CAP_NET_ADMIN` or run as root)
4. Credentials file format (if used): line 1 = username, line 2 = password

Test manually: `sudo openvpn --config /path/to/your.ovpn`

</details>

<details>
<summary><b>VPN keeps disconnecting</b></summary>

The bot auto-reconnects up to `VPN_MAX_RETRIES` times (default: 5). Check logs for the disconnect reason:

```bash
sudo journalctl -u tgbot -n 100 | grep VPN
```

Common causes: unstable VPN server, network issues, incorrect credentials.

</details>

<details>
<summary><b>How do I disable VPN temporarily?</b></summary>

```bash
python3 tg_downloader_bot.py --vpn=false
```

Or edit `.env` and set `VPN_ENABLED=false`, then restart.
